...
namespace raspberly.ovr
{
    public class FollowScript : MonoBehaviour
    {
        ...
        private UnityEngine.Quaternion rotDif;
        ...
        private void LateUpdate()
        {
            ...
            else transform.position = UnityEngine.Vector3.Lerp(transform.position, target.position, followMoveSpeed);

            rotDif = target.rotation * UnityEngine.Quaternion.Inverse(transform.rotation);
            rot = target.rotation;
            if (isLockX) rot.x = 0;
            if (isLockY) rot.y = 0;
            if (isLockZ) rot.z = 0;
            if (rotDif.w < rotateSpeedThreshold) transform.rotation = UnityEngine.Quaternion.Lerp(transform.rotation, rot, followRotateSpeed * 4);
            else transform.rotation = UnityEngine.Quaternion.Lerp(transform.rotation, rot, followRotateSpeed);
        }
